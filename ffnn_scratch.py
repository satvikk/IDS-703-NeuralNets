# hw_Feed-forward Neural Networks | Q2 | Submitted by Satvik Kishore, Deekshita Saikia
...
def main():
    ...
    cosine_similarity = (torch_results / np.linalg.norm(torch_results)) * (
        np_results / np.linalg.norm(np_results)
    ).reshape(-1)
    cosine_similarity = np.sum(cosine_similarity)
    print(
        "The cosine similarity between Numpy implementation and PyTorch implementation is: ",
